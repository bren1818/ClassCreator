...
function buildInclude(){
	...
	if( $('#headerfooter').prop('checked') == true ){
		code.val( code.val() + '\r\n');
		code.val( code.val() + tab(1) + 'function getHeader($title="") {\r\n');
...
/*
function incMultiDeleteFunction(){
	console.log("Adding Multi Delete");
	var fxString = "if (strtoupper($_SERVER['REQUEST_METHOD']) == 'POST'){ 
	if(isset($_POST['FX']) && $_POST['FX'] == 'multiDelete'){ $itemsToDelete = $_POST['items'];  } }";
...
function buildAdminForm(){
	...
	if( $('#headerfooter').prop('checked') == true ){
		...
		//	code.val( code.val() + incMultiDeleteFunction() );
		//}
		
		code.val( code.val() + tab(1) + 'getHeader("' + oName + ' Admin Page");\r\n');
	...
	if( $('#buildAjax').prop('checked') == true  ){
		...
		if( $('#multiDelete').prop('checked') == true ){
			...
			code.val( code.val() + ' <label>Muti Del: <input name="muiltiDelete[]" value="<?php echo $row["id"]; ?>" /></label>');
		}
	...
	*/
	...
	if( $('#multiDelete').prop('checked') == true ){ 
		code.val( code.val() + '<p style="text-align: right;"><select style="display: inline-block; width: auto; height: auto; padding: 0px;">');
		code.val( code.val() + '<option value="">-=Pick=-</option>');
		code.val( code.val() + '<option value="selectAll">Select All</option>');
	...
	if( $('#multiDelete').prop('checked') == true ){
		...
		code.val( code.val() + tab(3) +	'$("#processChecked").click(function(e){\r\n');	
		code.val( code.val() + tab(3) +	'	e.preventDefault();\r\n');	
		code.val( code.val() + tab(3) +	'	var action = $(this).parent().find("select > option:selected").val();\r\n');	
		code.val( code.val() + tab(3) +	'	var items = new Array();\r\n');	
		code.val( code.val() + tab(3) +	'	var selector = "table.dataTable  input[type=\'checkbox\'].multiSel";\r\n');	
			
		code.val( code.val() + tab(3) +	'	$("" + selector).each(function(index){\r\n');	
		code.val( code.val() + tab(3) +	'		if( action == "selectAll"){\r\n');	
		code.val( code.val() + tab(3) +	'			$(this).prop("checked", true);\r\n');	
		code.val( code.val() + tab(3) +	'		}else if(action == "selectNone"){\r\n');	
		code.val( code.val() + tab(3) +	'			$(this).prop("checked", false);\r\n');	
		code.val( code.val() + tab(3) +	'		}else if(action == "inverse"){\r\n');	
		...
		code.val( code.val() + tab(3) +	'			  resizable: false,\r\n');	
		code.val( code.val() + tab(3) +	'			  modal: true,\r\n');	
		code.val( code.val() + tab(3) +	'			  show: {\r\n');	
		code.val( code.val() + tab(3) +	'				effect: "blind",\r\n');	
		code.val( code.val() + tab(3) +	'				duration: 50\r\n');	
		code.val( code.val() + tab(3) +	'			  },\r\n');	
		code.val( code.val() + tab(3) +	'			  hide: {\r\n');	
		code.val( code.val() + tab(3) +	'				effect: "explode",\r\n');	
		code.val( code.val() + tab(3) +	'				duration: 50\r\n');	
		code.val( code.val() + tab(3) +	'			  }\r\n');	
		code.val( code.val() + tab(3) +	'			});\r\n');	
		...
		code.val( code.val() + tab(3) +	'				}\r\n');	
		code.val( code.val() + tab(3) +	'				$( "#dialog" ).dialog( "close" );\r\n');	
...
function buildCreateForm(){
	...
	if( $('#headerfooter').prop('checked') == true ){
		code.val( code.val() + tab(1) + 'getHeader("Create ' + oName + '");\r\n');
	}	
...
function buildUpdateForm(){
	...
	if( $('#headerfooter').prop('checked') == true ){
		code.val( code.val() + tab(1) + 'getHeader("Update ' + oName + '");\r\n');
	}	
...
function buildDeleteForm(){
	...
	if( $('#headerfooter').prop('checked') == true ){
		code.val( code.val() + tab(1) + 'getHeader("Delete ' + oName + '");\r\n');
	}	
	code.val( code.val() + '?>\r\n');
...
function buildAPICode(){
	...
	code.val('<?php\r\n');
	code.val( code.val() + '/*\r\n');
	code.val( code.val() + 'API Form Generated by Brendon Irwin\'s Form Generator\r\n');
	code.val( code.val() + 'Query Form: "' + oName + '" Object \r\n');
	code.val( code.val() + '\r\n');
	code.val( code.val() + '\r\n');
	code.val( code.val() + 'action' + oName + 'API.php\r\n');
	code.val( code.val() + '\r\n');
	code.val( code.val() + '*/\r\n');
	...
	code.val( code.val() + '		$items = isset($_POST["item"] ) ? $_POST["item"] : array();\r\n');
			
	code.val( code.val() + '	if( sizeof($items) > 0 ){\r\n');
	code.val( code.val() + '		$conn = getConnection();\r\n');
	code.val( code.val() + '		$count = 0;\r\n');
	code.val( code.val() + '		$list = "";\r\n');
	code.val( code.val() + '		$errors = array(); \r\n');
	...
	code.val( code.val() + '	}else{\r\n');
	code.val( code.val() + '		echo json_encode( array("error"=>"no items to process") ); \r\n');
	code.val( code.val() + '	}\r\n');
	code.val( code.val() + '}else{\r\n');
	code.val( code.val() + '		echo json_encode( array("error"=>"no data submitted") ); \r\n');
	code.val( code.val() + '}\r\n');
	...
	code.val(code.val() + '?>');
	makeCodeEditor('GenAPICode', "text/x-php");
}
	...
	code.val( code.val() + tab(2) +	'if( $orderBy != "" && is_int($orderBy) ){\r\n');
		...
		for(var obc = 0; obc < variables.length; obc++){
			...
			if( variables[obc].showAdmin == true ){
				...
				console.log( variables[obc] );
				//check showAdmin
				
				
				//check it is not SELCECT MULTIPLE
				if( variables[obc].type != 'selectMultiple' ){
